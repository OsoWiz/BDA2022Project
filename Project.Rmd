...
## State
State tells the state in the US in which the tornado happened. This parameter functions as the grouping in our hierarchical model.

## Magnitude
This is the magnitude of the tornado on the enhanced fujita scale, which is based on the windspeed of the tornade. It is the covariate of our model.
...
We decided to try linear regression in our analysis. Our dataset contained information of tornadoes in the United States in years 1950-2021. We divided the dataset into groups by the state in which the torando occurred. Then we calculated the average injuries caused by tornadoes of a certain magnitude in each state. Our hypothesis was that the average of injuries was linearly proportional to the magnitude of a tornado.

We developed to models for the linear regression: a pooled model and a hierarchial model. The pooled model can be applied if the injuries are considered to be indentically and independetly distributed between the states. The hierarchial model allows the injuries to be dependent on the state of the torando's occurence.
...
data_pooled <- readRDS(file='~/notebooks/BDA/Project/data_pooled_ver2.Rda')
data_hx <- readRDS(file='~/notebooks/BDA/Project/data_hierarchical_x_ver2.Rda')
data_hy <- readRDS(file='~/notebooks/BDA/Project/data_hierarchical_y_ver2.Rda')
...
A better formulation of our data would have perhaps functioned better than this one. Some model that would have given more datapoints per grouping could have gotten more out of it. That could have potentially fixed the linearity problem as well.
